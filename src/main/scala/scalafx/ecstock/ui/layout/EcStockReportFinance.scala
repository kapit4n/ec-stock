...
class EcStockReportFinance extends EcStockExample {
  ...
  def buildChart(revenueValues: HashMap[String, Number]): BarChart[String, Number] = {
  ...
  def getContent = {
   val todayFilter = new Button(Messages.data("today")) {
      onAction = (ae: ActionEvent) => {
        var chart = buildChart(DBManager.getFinanceReport(DBManager.TODAY))
    ...
    val lastMonthFilter = new Button(Messages.data("thisMonth")) {
      onAction = (ae: ActionEvent) => {
        var chart = buildChart(DBManager.getFinanceReport(DBManager.LAST_MONTH))
        childrenVBox.children = List(filtersGrid, chart)
      }
    }
    ...
    val allFilter = new Button(Messages.data("all")) {
      onAction = (ae: ActionEvent) => {
        var chart = buildChart(DBManager.getFinanceReport(DBManager.ALL))
        childrenVBox.children = List(filtersGrid, chart)
      }
    }
    ...
    val rangeFilter = new Button(Messages.data("searchRange")) {
      onAction = (ae: ActionEvent) => {
        ...
        var chart = buildChart(DBManager.getFinanceReport(DBManager.RANGE))
        childrenVBox.children = List(filtersGrid, chart)
