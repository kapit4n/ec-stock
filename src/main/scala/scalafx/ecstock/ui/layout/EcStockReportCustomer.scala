...
class EcStockReportCustomer extends EcStockExample {
  ...
  def buildChart(customerValues: HashMap[String, Number]): BarChart[String, Number] = {
    val customerNames = customerValues.keySet.toList
    ...
    for (customerName <- customerNames) totalBuf += customerValues(customerName)
  ...
  def getContent = {
    ...
    val todayFilter = new Button(Messages.data("today")) {
      onAction = (ae: ActionEvent) => {
        var chart = buildChart(DBManager.getCustomerReport(DBManager.TODAY))
    ...
    val lastMonthFilter = new Button(Messages.data("thisMonth")) {
      onAction = (ae: ActionEvent) => {
        var chart = buildChart(DBManager.getCustomerReport(DBManager.LAST_MONTH))
        childrenVBox.children = List(filtersGrid, chart)
      }
    }
    ...
    val allFilter = new Button(Messages.data("all")) {
      onAction = (ae: ActionEvent) => {
        var chart = buildChart(DBManager.getCustomerReport(DBManager.ALL))
        childrenVBox.children = List(filtersGrid, chart)
      }
    }
    ...
    val rangeFilter = new Button(Messages.data("searchRange")) {
      onAction = (ae: ActionEvent) => {
        ...
        var chart = buildChart(DBManager.getCustomerReport(DBManager.RANGE))
        childrenVBox.children = List(filtersGrid, chart)
